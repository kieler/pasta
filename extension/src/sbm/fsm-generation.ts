...
import { askForPath, collectContextVariables } from "./sbm-generation";
...
export async function createFSMs(
    ...
    for (const controller of Object.keys(controlActionsMap)) {
        await createControllerFSM(controller, controlActionsMap[controller], formulaMap[controller] ?? []);
    }
}
 ...
 * @returns
 */
export async function createControllerFSM(
    ...
    // create the scchart
    const scchartText = createFSMSCChart(
        controllerName,
        states,
        variables,
